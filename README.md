...
- **Zoroastrianism** as a third faith layer. The best-fitting candidate on the list: a known Iranian-plateau core, a documented decline after the Arab conquest of 651, and the Parsi migration to Gujarat and later Bombay. Bulliet's onomastic conversion curves put Iran's tipping point around the mid-9th century and near-complete conversion by c. 1000, and they are logistic curves, which is the shape this engine's simulation already produces. Pre-conquest share is the soft number: Sasanian Iran also held large Christian, Jewish, Manichaean, and Buddhist populations, so "majority Zoroastrian" needs a stated band rather than a figure.
- **Pre-Columbian and Native American layers, but not as a share-of-population metric.** Share is degenerate before contact (a region is effectively 100% its own tradition), so the honest variables are polity extent, population, and movement. What the existing engine could carry with real confidence: Maya and Aztec cities on the city-lifecycle machinery, since Long Count stelae date Tikal, Calakmul, Copán, and Palenque political history precisely and the Codex Mendoza tribute lists give Aztec imperial extent; Cahokia's rise around 1100 and abandonment by c. 1350; and dated movements on the journey-rendering engine (Iroquois expansion in the Beaver Wars of the 1640s, Comanche onto the southern plains after c. 1700, Lakota west out of Minnesota through the later 1700s, the removals of 1831 to 1850).
- The blocker on the above is the population baseline, not the geography. Central Mexico in 1519 is estimated anywhere from ~4.5M (Rosenblat) to ~25M (Borah and Cook), and hemisphere totals span ~8M to ~112M, so any absolute figure rendered as a single number would be the most precision-implying thing in the project. Contact-era tribal territory maps carry the same trap in a different form: they are snapshots sold as timeless, and they flatten centuries of displacement. Either would need a rendered uncertainty band rather than a value, which is a rendering feature the engine does not have yet.
