...
- **Zoroastrianism** as a fourth faith, and the best-fitting dataset candidate on this list. It has a known Iranian-plateau core, a documented decline after the Arab conquest of 651, and the Parsi migration to Gujarat and later Bombay, so it needs a registry entry and a share table rather than anything in the renderer. Bulliet's onomastic conversion curves put Iran's tipping point around the mid-9th century and near-complete conversion by c. 1000, and they are logistic curves, which is the shape the simulation already produces. It would also be a hard test of the confidence tiering, spanning conjecture (the Sasanian share, in an empire that also held large Christian, Jewish, Manichaean and Buddhist populations) and attested (modern Parsi counts, small enough to vanish under the world-share ramp) inside a single curve.
- **Pre-Columbian and Native American layers, but as a measure rather than a faith.** Share of population is degenerate before contact, since a region is effectively 100% its own tradition, so the honest variables are polity extent, population and movement. What the engine could carry with real confidence today: Maya and Aztec cities on the city-lifecycle machinery, since Long Count stelae date Tikal, Calakmul, Copán and Palenque precisely and the Codex Mendoza tribute lists give Aztec imperial extent; Cahokia's rise around 1100 and abandonment by c. 1350; and dated movements on the parked journey renderer, which is still in the file (Iroquois expansion in the Beaver Wars of the 1640s, Comanche onto the southern plains after c. 1700, Lakota west out of Minnesota through the later 1700s, the removals of 1831 to 1850).
- The blocker on the population side of the above is the baseline, not the geography, and the confidence wash only half covers it. Central Mexico in 1519 is estimated anywhere from ~4.5M (Rosenblat) to ~25M (Borah and Cook), and hemisphere totals span ~8M to ~112M. Marking that *conjecture* and *contested* says the figure is poorly known; it does not say that the honest answer is a fivefold range, which is a different rendering problem from the one the tiers solve. Contact-era tribal territory maps carry the same trap in another form: they are snapshots sold as timeless, and they flatten centuries of displacement.
